...
const getContact = async (req, res) => {
    try {
        const contact = await Contacts.find({user: mongoose.Types.ObjectId(req.query.userId)})
            .select({customers: {$elemMatch: {_id: mongoose.Types.ObjectId(req.query.contactId)}}});
        res.json(contact);
    } catch (err) {
        console.log(err);
        res.status(500).send({ message: err });
